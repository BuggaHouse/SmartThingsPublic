...
definition(
	name: "Papertrail HTTP Event Logger",
	namespace: "Caplaz",
...
preferences {
	...
	section ("Papertrail Server") {
		...
		input "papertrail_token", "text", title: "Papertrail Authentication Token", required: true
	}
}
...
def genericHandler(evt) {
	...
	*/
	...
	if (local == true) {
		...
		def params = [
			uri: "${http_protocol}://${papertrail_host}:${papertrail_port}/v1/log",
			headers: [ 'Authorization': "Basic ${papertrail_token}"],
			body: json
		]
